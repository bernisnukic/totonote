...
26. **Never size TipTap's node-view wrapper to fit its contents.** A node decoration lands on `.react-renderer.node-<name>`, which is always column-width, so outlining a narrow drawing drew a full-width box. The obvious fix — `width: fit-content` on the wrapper — is wrong twice over: `fit-content` cannot measure a child that is `width: 100%`, so the wrapper collapsed to the width of the *Draw button* beside it (54px); and both `DrawingNodeView` and `ResizableImage` read their maximum drag width from an ancestor, so that 54px became the resize ceiling and no drag could ever give width back. Pass the colour down as `--annotation-color` / `--annotation-tint` instead and outline the drawing itself — no layout change, so it cannot disturb resizing. The drag ceiling comes from `lib/column-width.ts`, measured off the editor column, which does not move when the thing inside it does. Guarded by `e2e/drawing-resize.spec.ts`.
27. **A duplicate key in an object literal is silent, and the last one wins.** A `handleClick` added to `editorProps` alongside the one already there simply never ran — and three rounds of reasoning about how ProseMirror resolves click positions went into explaining a handler that did not exist. Before adding a handler to a large literal, grep the literal for that key. When a handler "isn't firing", put a `console.log` in it and read the renderer console before theorising about the framework: one instrumented run beat three careful arguments.
28. **`fit-content` on `.react-renderer` is safe now, and was not before.** The wrapper is column-width, so the empty space beside a narrowed drawing belonged to the drawing — clicks and right-clicks there hit it. Shrink-wrapping the wrapper is what broke resizing in 1.21.2 (pitfall 26); it works now only because three things changed: `.drawing-node` carries an explicit pixel width instead of `width: 100%`, so there is something to measure; the drag ceiling comes from `lib/column-width.ts` rather than an ancestor; and the tag outline is drawn on the drawing itself. Change any one of those back and the collapse returns. Gated on `.is-sized`, because an untouched drawing fills the column and must keep doing so.
29. **The splash animation plays once, and the loop count is not how you get that.** `assets/splash/intro.gif` has its NETSCAPE application extension **removed entirely** — a GIF with no loop extension plays once and stops on its last frame. Setting the loop count to `1` looks like it should mean the same thing and does not: Chromium reads it as "repeat once more" and plays the animation twice. That shipped in 1.23.1 and the splash closed partway through the second run; the test that was supposed to guard it asserted `loopCount === 1` and passed, which was worse than having no test. `splash-timing.test.ts` now asserts the extension is absent, and reads the frame delays out of the file so the constants in `splash-timing.ts` cannot drift from the animation. The one figure they replaced was 3150ms against a 3300ms animation, cutting five frames off every launch.
30. **The splash window is transparent, so nothing may fade the card itself.** Fading `.splash` from zero made the whole window see-through at the start and whatever sat behind it showed through — reported as "a white border instead of all black". The card is opaque black from the first paint; only `.contents` fades. For the same reason the fade lands on `intro-first.png`, a still of frame 1: a GIF starts the instant it has a source, so the animation is withheld until the fade ends, and without the still the fade arrives at an empty box. `verify-package.mjs` requires all three files in the packaged app.
31. **The splash belongs to launching the app, not to opening a window.** On macOS `window-all-closed` deliberately does not quit, so 'activate' builds a fresh window through `createWindow()` — which ran the splash again every time. A `splashShown` flag in `index.ts` keeps it to once per process. The guard in `startup.spec.ts` is macOS-only by construction: everywhere else closing the last window quits, so the case cannot arise. Verified by removing the flag and watching the test see a 420px window where the app should be.
32. **A slow E2E suite is usually a failing one.** Nine tests each burning a 30s timeout, twice with retries, is nine minutes — enough to look exactly like a global slowdown and send you hunting Chromium throttling flags. Check the failure count and read one actual error before theorising about performance; the totals not reconciling (159 passed of 168 defined) is the tell.
