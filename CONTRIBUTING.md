...
29. **The splash animation plays once, deliberately.** `assets/splash/intro.gif` has its NETSCAPE loop count patched from 0 (forever) to 1, because a looping mark has no last frame to pause on — re-export it from almost any tool and it comes back looping, at which point the pause silently becomes "start again". Its timings are not guessed: `splash-timing.ts` states the fade, the measured animation length and the pause separately, and `splash-timing.test.ts` reads the frame delays and the loop flag out of the GIF and fails if they disagree with the constants. The single figure it replaced was 3150ms against a 3300ms animation, so five frames were cut off every launch. The fade works by withholding the `<img src>` until the fade-in animation ends — a GIF starts the instant it has a source, so fading a card in around one that already has its source just reveals it mid-play.
30. **A slow E2E suite is usually a failing one.** Nine tests each burning a 30s timeout, twice with retries, is nine minutes — enough to look exactly like a global slowdown and send you hunting Chromium throttling flags. Check the failure count and read one actual error before theorising about performance; the totals not reconciling (159 passed of 168 defined) is the tell.
